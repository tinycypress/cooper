...
export default class CratedropMinigame {
    ...
    static async open(reaction) {
        ...
        setTimeout(async () => {
            ...
            // Send the update.
            COOP.CHANNELS.propagate(msg, crateLootText, 'ACTIONS');
    ...
    static async drop() {
        try {
            ...
            // Drop the crate!
            const random = await COOP.CHANNELS._randomSomewhatActive();
            if (random) {
                ...
                const feedMsg = await COOP.CHANNELS._postToChannelCode('ACTIONS', `${rarityWord} crate drop in progress.`);
